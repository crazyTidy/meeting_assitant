...
"""
    ...
    def _build_prompt_from_timeline(
        ...
        【Markdown格式规范】
        1. 文档标题必须使用一级标题（#），格式为：# {meeting_title}
        2. 标题层级最多使用四级（####），严禁使用五级（#####）和六级（######）标题
        3. 标题层级要合理递进，不要跳级（如从 # 直接跳到 ###）

        【输出要求】
        - 标题明确：直接使用一级标题（#）输出会议核心标识或优化后的精准标题（如”# 关于XX项目推进的会议纪要”）；
