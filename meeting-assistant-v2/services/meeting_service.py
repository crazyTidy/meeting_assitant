...
from ..items.meeting_item import MeetingListResponseItem, MeetingStatusResponseItem
from ..repositories.meeting_repository import (
    meeting_repository,
    participant_repository,
    summary_repository
)
from ..tasks.processor import process_meeting_task
from ..tasks.regenerator import regenerate_summary_task
...
class MeetingService:
    ...
    async def create_meeting(
        ...
        unique_filename = f"{uuid.uuid4()}{Path(audio_file.filename).suffix}"
        upload_dir = Path(environment_config.UPLOAD_DIR)
        ...
        async with aiofiles.open(file_path, 'wb') as f:
            ...
            if len(content) > environment_config.MAX_UPLOAD_SIZE:
                raise ValueError(
                    f"文件过大。最大允许: {environment_config.MAX_UPLOAD_SIZE // 1024 // 1024}MB"
                )
